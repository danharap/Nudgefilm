...
function isShowTab(v: string | null): v is ShowTab {
  return !!v && (SHOW_TABS as readonly string[]).includes(v);
}
...
export function ShowTabsPanel({
  ...
  const activeTab: ShowTab = isShowTab(rawTab) ? rawTab : "cast";

  function tabHref(tab: ShowTab) {
    const sp = new URLSearchParams(searchParams.toString());
    sp.set("tab", tab);
    return `/show/${slug}?${sp.toString()}`;
  }
    ...
    <section className="mt-8 sm:mt-10">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {cast.slice(0, 36).map((member) => (
            <Link
              ...
              prefetch={false}
              className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5 text-sm text-zinc-200 hover:border-indigo-400/30"
            >
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            ...
            {directors.length > 0 ? directors.map((d) => (
              <Link key={`d-${d.id}`} href={`/person/${d.id}`} prefetch={false} className="block text-sm text-zinc-200 hover:text-indigo-300">{d.name}</Link>
      ...
      {activeTab === "details" ? (
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3 text-sm text-zinc-300">First air date: {firstAirDate || "—"}</div>
      ...
      {activeTab === "genres" ? (
        <div className="mt-4 flex flex-wrap gap-2">
          {genres.map((g) => (
            <Link key={g.id} href={`/recommend?source=tv&genres=${g.id}`} prefetch={false} className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-sm text-zinc-200 hover:border-indigo-400/30">
              {g.name}
            </Link>
          ))}
        </div>
